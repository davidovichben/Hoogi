...
@Component({
  ...
  template: `
    <div class="p-6 max-w-xl mx-auto">
      <div *ngIf="questionnaire?.meta?.brand_logo_url" class="mb-3">
        <img [src]="questionnaire?.meta?.brand_logo_url" alt="logo" class="h-10">
      </div>
      <h1 class="text-2xl font-semibold mb-3">{{ questionnaire?.title }}</h1>

      <!-- Display questions with advanced styling -->
      <div *ngIf="questions && questions.length > 0" class="space-y-4">
        <div *ngFor="let question of questions; let i = index"
             class="rounded-xl border p-5 bg-white/60 backdrop-blur-sm">
          <div class="flex items-center justify-between mb-3">
            ...
            <span class="text-xs text-gray-400 capitalize">{{ question.type }}</span>
          </div>

          <h3 class="font-medium mb-3 text-gray-800">{{ question.title }}</h3>

          <!-- Question type: text -->
          <input *ngIf="question.type === 'text'"
                 type="text"
                 placeholder="הקלד תשובתך כאן..."
                 class="w-full p-3 border rounded-lg bg-white/80 backdrop-blur-sm"
                 disabled>

          <!-- Question type: audio -->
          <div *ngIf="question.type === 'audio'"
               class="w-full p-4 border rounded-lg bg-white/80 backdrop-blur-sm">
      ...
      <div *ngIf="!questions || questions.length === 0"
           class="rounded-xl border p-5 bg-white/60 text-center text-gray-500">
        אין שאלות להצגה
      </div>
    </div>
  `
