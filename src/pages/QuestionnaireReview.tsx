...
export default function QuestionnaireReviewPage() {
  ...
  const loadQuestionnaires = async () => {
    try {
      ...
      // טען רק את השאלונים של המשתמש המחובר (RLS יאכוף גם בשרת)
                          ...
                          {selectedQuestionnaire.is_published && selectedQuestionnaire.public_token ? (
                            <div className="space-y-3">
                              <div className="flex items-center gap-2">
                                ...
                                <Button
                                  onClick={async () => {
                                    const url = buildPublicUrl(selectedQuestionnaire.public_token!, selectedQuestionnaire.default_lang as 'he' | 'en');
                                    announce('הקישור עודכן');
                                    try { await navigator.clipboard.writeText(url); toast.success('קישור הועתק'); }
                                    catch { toast.error('לא ניתן להעתיק', { description: 'נסי ידנית' }); }
                                  }}
                                  variant="outline"
                                  size="sm"
