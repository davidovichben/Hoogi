...
אתה יועץ UX וכתיבת שאלונים.

חשוב: אתה כותב שאלון שמוצג ללקוח/ת קצה שפונה לעסק כדי לקבל שירות.
אל תשאל על העסק עצמו – שאל רק את הלקוח הפונה כדי להבין את הצורך שלו/ה.
...
דרישות:
1) לכל שאלה ציין "סוג שאלה" אחד מהמחרוזות המדויקות:
   ["בחירה יחידה","בחירה מרובה","שדה טקסט חופשי","כן/לא"].
   אם זה סוג בחירה – ספק לפחות 3 אופציות קצרות וברורות.
2) ניסוח ידידותי, קצר, מותאם לנייד.
3) אל תכלול שאלות פנימיות על העסק (כמו "מה תחום העסק שלך").
4) אם חסר מידע – שאל שאלות כלליות אך שימושיות ללקוח פונה.

פלט נדרש:
החזר אך ורק JSON במבנה הבא (שימי לב – בלי טקסט מסביב, רק JSON):
[
  {"text":"...שאלת לקוח...","type":"בחירה יחידה","options":["אופציה 1","אופציה 2","אופציה 3"]},
  {"text":"...שאלת לקוח...","type":"שדה טקסט חופשי"}
]
`.trim();
